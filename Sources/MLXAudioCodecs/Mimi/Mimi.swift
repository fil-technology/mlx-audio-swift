...
public final class Mimi: Module {
    ...
    public func resetState() {
        encoder.resetState()
        decoder.resetState()
        // Rebuild the KV caches outright. `c.trim(c.offset)` only zeroes the
        // logical offset; the underlying MLXArray storage keeps its shape and
        // some attention paths read `.dim(2)` directly, which causes
        // `broadcast_shapes` crashes when the model instance is reused across
        // generate() calls.
        encoderCache = encoder_transformer.makeCache()
        decoderCache = decoder_transformer.makeCache()
    }

    /// Replace the decoder-side KV cache with a fresh, empty instance.
    /// Exposed for `MimiStreamingDecoder` and any external caller that needs
    /// to reset decoder state without also rebuilding the encoder cache.
    public func resetDecoderCache() {
        decoderCache = decoder_transformer.makeCache()
    }
