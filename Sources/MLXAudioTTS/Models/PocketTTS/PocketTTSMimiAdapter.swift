...
public final class MimiAdapter: Module {
    ...
    public func resetState() {
        ...
        upsample?.resetState()
        // Rebuild the KV caches outright instead of `c.trim(c.offset)`, which
        // only resets the logical offset and leaves stale storage shape on the
        // underlying MLXArrays. When the model instance is reused across
        // generate() calls (which TTSMLX does to amortize load cost), the
        // stale shape causes attention `broadcast_shapes` mismatches like
        // `(1,8,16,64) vs (1,8,32,64)` because some attention paths read
        // `keys.dim(2)` rather than `cache.offset`.
