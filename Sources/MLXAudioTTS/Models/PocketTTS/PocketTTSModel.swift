...
public final class PocketTTSModel: Module, SpeechGenerationModel, @unchecked Sendable {
    ...
    public func generate(
        ...
        // Defense-in-depth: ensure mimi starts each generate() from a clean
        // slate. The per-sentence path inside generateAudioStreamShortText
        // also resets, but resetting at the entry point guarantees correct
        // behavior regardless of how callers stitch generations together
        // (e.g. when the model instance is reused across calls, which is the
        // common case once TTSMLX caches loaded weights).
        mimi.resetState()
